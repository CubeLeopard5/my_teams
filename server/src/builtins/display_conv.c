...
char *check_if_conv_exists(char *uuid1, char *uuid2) {
    char *file_name = concat(concat(concat(uuid1, "_"), uuid2), ".txt");
    char *path_file = concat(CONV_DIR, file_name);
    char *file_name_reversed = concat(concat(concat(uuid2, "_"), uuid1), ".txt");
...
void display_conv(server_t *server, size_t client_nbr, char **command) {
    ...
    if (get_nb_word(command) != 2) {
        ...
        if (conv_file == NULL) {
            send_message_to_client(server, client_nbr, "No conversation found");
        } else {
